...
class data_transformation():
    ...
    dbscan - if True add vector with cluster according to dbscan used for splitting
    transformation - if log then do the log transformation on the target
    '''
    def __init__(self, path_data, path_mhc, allele_name = None, quant_data = True, encoding = "one-hot", dbscan = True, transformation = 'log'):
...
class data_class_test(unittest.TestCase):
    def setUp(self):
        self.class_data = data_transformation(path_data = "/data/data_curated_20180219/curated_training_data_no_mass_spec_dbscan.csv",
